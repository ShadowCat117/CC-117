...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription(
            'Adds a player to be excluded from promotion checks. Default length is forever.',
        )
        .addStringOption((option) =>
            option
                .setName('username')
                .setDescription(
                    'The name of the player you want to be exemept from promotion checks.',
                )
                .setRequired(true),
        )
        .addIntegerOption((option) =>
            option
                .setName('duration')
                .setDescription(
                    'How long should they be exempt from promotion?',
                ),
        ),
    ephemeral: true,
    async execute(interaction) {
        ...
        try {
            ...
            if (response.playerUuids !== undefined) {
                ...
                // Error whilst trying to add promotion exception
                errorEmbed
                    .setTitle('Error')
                    .setDescription(
