...
module.exports = {
	...
	async execute(interaction) {
		...
		if (response.componentIds.length > 0) {
			...
			interaction.editReply({ content: `No activity data found for ${interaction.options.getString('guild_name')}`, components: [] });
		} else {
			const row = new ActionRowBuilder();
			...
			await interaction.editReply({ content: response.pages[0], components: [row] });
		}
	},
};