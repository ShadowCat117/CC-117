    ...
    try {
        ...
        if (player && player.message === 'Multiple possibilities found') {
            ...
            const filteredPlayers = player.playerUuids
                ...
                .filter(({ playerUuid }) =>
                    Object.keys(config.demotionExceptions).includes(playerUuid),
                );
        ...
        if (
            ...
            !config['demotionExceptions'][player.uuid]
        ) {
            return {
                error: `${player.username.replaceAll('_', '\\_')} is not exempt from demotions.`,
            };
        }
